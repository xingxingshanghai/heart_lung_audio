...
public class AudioPlayerHandler implements Runnable {
    ...
    private int frequence = 4000;// 采样率 8000
    private int channelInConfig = AudioFormat.CHANNEL_OUT_MONO;// 定义采样为单声道（过时，但是使用其他的又不行
    private int audioEncoding = AudioFormat.ENCODING_PCM_16BIT;// 定义音频编码（16位）
    private int bufferSize = -1;// 播放缓冲大小
    private LinkedBlockingDeque<Object> dataQueue = new LinkedBlockingDeque<Object>();
    // 互斥信号量
    ...
    public AudioPlayerHandler() {
        ...
        // 实例AudioTrack
        track = new AudioTrack(AudioManager.STREAM_MUSIC, frequence, channelInConfig, audioEncoding, bufferSize*4, AudioTrack.MODE_STREAM);
